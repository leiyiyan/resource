...
更新日期：2024.08.06 10:39:00

茶百道自动获取ck并同步到青龙后自动执行脚本，用于解决茶百道 Cookie 有效期过短，需要频繁抓取的问题
...
2.添加该脚本到重写，并配置好 hostname；
3.配置好boxjs订阅，并按相关提示配置好参数；
    青龙服务地址：http://xx.xx.xx.xx:5700，注意结尾不要带 "/"
    clientId：第 1 部中创建的应用的 Client ID
    secret：第 1 部中创建的应用的 Client Secret
    envName：要同步的环境变量名称
4.打开茶百道小程序->点单页面，自动获取 Cookie；
5.运行后可自行选择关闭获取脚本，防止产生不必要的mitm。

[Script]
http-request ^https?:\/\/apisix\-gateway\-pro\.shuxinyc\.com\/decrypt script-path=https://raw.githubusercontent.com/leiyiyan/resource/main/script/cbd/cbd_ck.js, requires-body=false, timeout=60, tag=茶百道同步青龙

[MITM]
hostname = apisix-gateway-pro.shuxinyc.com

*/
// env.js 全局
const $ = new Env("茶百道同步青龙");//脚本名称
// 调试
$.is_debug = ($.isNode() ? process.env.IS_DEDUG : $.getdata('is_debug')) || 'false';
// 为通知准备的空数组
$.notifyMsg = [];
//青龙配置
let QL = ($.isNode() ? process.env.CBD_QL : $.getjson("CBD_QL")) || {};
//---------------------- 自定义函数区 -----------------------------------
