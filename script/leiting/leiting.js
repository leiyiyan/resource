const $ = new Env("雷霆游戏会员");
const ckName = "leiting_data";
//-------------------- 一般不动变量区域 -------------------------------------
...
// 为通知准备的空数组
$.notifyMsg = [];
const postIds = ["1827205", "1826541", "1825846"]
    ...
    try {
        for (let user of userList) {
            ...
            if (user.ckStatus) {
                ...
                await $.wait(user.getRandomTime());
                console.log("📖 任务2：每天浏览3篇帖子")
                for(let i = 0; i < postIds.length; i++) {
                    await user.viewPost(postIds[i])
                    await $.wait(user.getRandomTime());
                }
                console.log("📖 任务3：每天累计点赞3篇帖子")
                for(let i = 0; i < postIds.length; i++) {
                    await user.likePost(postIds[i], "like")
                    await $.wait(user.getRandomTime());
                    await user.likePost(postIds[i], "unlike")
                    await $.wait(user.getRandomTime());
                }
                console.log("📖 任务4：每天分享1篇帖子")
                await user.sharePost(postIds[0])
                await $.wait(user.getRandomTime());
...
class UserInfo {
    ...
    async sign() {
        try {
            const timestamp = Date.now()
            const opts = {
                url: `/activity/outer/activity/template/game_sign/352668398326423552/sign`,
                type: "get",
                headers: Object.assign(this.headers, {
                    timestamp,
                    sign: getSign(timestamp)
    ...
    async getSignInfo() {
        try {
            const timestamp = Date.now()
            const opts = {
                url: `/activity/outer/activity/template/game_sign/352668398326423552/home`,
                type: "get",
                headers: Object.assign(this.headers, {
                    timestamp,
                    sign: getSign(timestamp)
            ...
            let res = await this.fetch(opts);
            if (res?.code == 2000) {
                const { signDays, signTaskList } = res?.data;
                if(signDays == 3) {
                    console.log(`✅ 本周已签到${signDays}天，可领取：${signTaskList[0]?.award?.name}`);
                    await this.receiveReward(3, signTaskList[0]?.award?.name);
                }
                else if(signDays == 5) {
                    console.log(`✅ 本周已签到${signDays}天，可领取：${signTaskList[1]?.award?.name}`);
                    await this.receiveReward(5, signTaskList[1]?.award?.name);
                }
                else if(signDays == 7) {
                    console.log(`✅ 本周已签到${signDays}天，可领取：${signTaskList[2]?.award?.name}`);
                    await this.receiveReward(7, signTaskList[2]?.award?.name);
                }
            }else {
                console.log("⛔️ 查询本周签到天数失败：" + res.msg);
            }
        } catch (e) {
            this.ckStatus = false;
            $.log(`⛔️ 查询本周签到天数失败! ${e}`);
    ...
    // 领取签到奖励
    async receiveReward(day, name) {
        try {
            const timestamp = Date.now()
            const opts = {
                url: `/activity/outer/activity/template/game_sign/352668398326423552/receive?dayLimit=${day}`,
                type: "get",
                headers: Object.assign(this.headers, {
                    timestamp,
                    sign: getSign(timestamp)
            ...
            let res = await this.fetch(opts);
            if (res?.code == 2000) {
                console.log(`✅ 成功领取第${day}天签到奖励：${name}`);
            }else {
                console.log(`⛔️ 领取第${day}天签到奖励失败：` + res.msg);
            }
        } catch (e) {
            this.ckStatus = false;
            $.log(`⛔️ 领取第${day}天签到奖励失败! ${e}`);
        }
    }
    // 浏览帖子
    async viewPost(postId) {
        try {
            ...
            if (res?.code == 2000) {
                ...
                console.log("⛔️ 浏览帖子失败：" + res.msg);
            }
        } catch (e) {
            this.ckStatus = false;
            $.log("⛔️ 浏览帖子失败：" + res.msg);
        }
    }
    // 点赞
    ...
    async launchGame() {
        try {
            const timestamp = Date.now()
            const opts = {
                url: `/app/game/1/launch/report/mission`,
                type: "get",
                headers: Object.assign(this.headers, {
                    timestamp,
                    sign: getSign(timestamp)
...
function getSign(timestamp) {
    ...
    const timespan = timestamp.toString()
    return $.CryptoJS.MD5(device + timespan).toString()
}
async function getCookie() {
    ...
    const index = userCookie.findIndex(e => e.userName == newData.userName);
...
async function checkEnv() {
    try {
        ...
        throw new Error(`⛔️ checkEnv run error => ${e}`)
    }
}
//请求二次封装
async function Request(o) {
    ...
    try {
        ...
        const httpPromise = $.http[method.toLowerCase()](request)
            ...
            .catch(err => $.log(`⛔️ 请求发起失败！原因为: ${err}`));
